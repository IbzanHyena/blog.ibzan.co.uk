...
Amongst the number of languages I have learnt to write, it is one of only two I have used with daily consistency for so long—and the other one, Python, is part of my day job.
...
Compared to other programming languages, J uses some different terms to refer to its constituent parts.
Briefly:
...
<span class="lining">Also, to be clear, all arrays in J are flat and rectangular.
This means an array can have any number of dimensions, but each row of a 2D array must have the same number of elements, and each layer of a 3D array is a 2D array of the same shape, and so forth.</span>
...
Combined with the standard ability of array languages to manipulate entire arrays at once, J is a language of incredible brevity.

[Tacit programming][tacit] is a paradigm where explicit names for arguments and variables are omitted, usually because the nature of the language being written precludes the need for them.
...
Fundamental programming skills like how to structure a program or break one down into constituent parts haven’t gone away, but when an algorithm fits on one line, the functions you build can be more complex.
...
I believe this has emerged because J opted to use ASCII for its primitives, rather than the more typical symbology found in [APL][APL], [BQN][BQN], [uiua][uiua], and others.
Simply put, there aren’t enough good Unicode glyphs to make some of the primitives J uses.
While many of J’s primitives are basic and could be built out of other ones, providing the more complete set can make writing code faster, in my experience.

The way that J tackles the symbol problem is by allowing its operators to be multiple characters.
...
The breadth of primitives segues nicely to what I consider to be one of J’s strongest assets—a willingness to carry more complexity in the language in a sensible way.
...
By giving the programmer the ability to interact with more types, J trades away some of the pure simplicity of a single numeric type for enhanced versatility.
...
At first, this explicit opt-in behaviour by changing the source code may seem unwieldy or opaque, but it’s a side effect of the simplicity of the interpreter’s control loop, which has benefits in other areas when considered on the whole.
...
One of the chief ones comes in its most popular descendants—NumPy and PyTorch—both of which are heavily based on the J (and APL) array models.
...
Using J, I’ve managed to efficiently prototype a number of algorithms for my job as a data scientist and then translated them into PyTorch or NumPy to be actually used.
...
It’s fair to say that there are limits to what the array paradigm is good at solving; I still use other languages for many tasks that are not well-reduced to processing data algorithmically.

Perhaps the simplest way of putting it is that J is an analytical language, not an operational one.
...
I learnt a lot of new ways (the array paradigm and mindset, tacit/point-free programming, and less reliance on thorough typing, to name a few) to think about software by getting to grips with it.
...
I’m looking forward to exploring these in more detail to deepen my understanding of the language.
...
[nouns]: https://code.jsoftware.com/wiki/Vocabulary/Nouns
[nuvoc]: https://code.jsoftware.com/wiki/NuVoc
[parts-of-speech]: https://code.jsoftware.com/wiki/Vocabulary/PartsOfSpeech
